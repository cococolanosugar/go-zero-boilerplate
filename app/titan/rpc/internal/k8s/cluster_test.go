...
func TestClusterManagerOperations(t *testing.T) {
	...
	err := cm.EnsureNamespace(ctx, "titan-test")
	if err != nil {
	...
	if !found {
		t.Fatalf("expected namespace titan-test in %v", namespaces)
	}
	...
	_, err = fakeClient.AppsV1().Deployments("titan-test").Create(ctx, deploy, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Create deployment failed: %v", err)
	}

	ready, msg, err := cm.CheckDeploymentReady(ctx, "titan-test", "order-service")
	if err != nil {
