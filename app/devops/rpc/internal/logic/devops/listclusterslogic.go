...
func (l *ListClustersLogic) ListClusters(in *devops.ListClustersReq) (*devops.ListClustersResp, error) {
	page := in.Page
	if page <= 0 {
		page = 1
	}
	...
	var total int64
	countQuery := fmt.Sprintf("SELECT COUNT(*) FROM devops_cluster %s", where)
	if err := l.svcCtx.SqlConn.QueryRowCtx(l.ctx, &total, countQuery, args...); err != nil {
		return nil, err
	}

	var clusters []*model.DevopsCluster
	listQuery := fmt.Sprintf("SELECT id, name, env, api_endpoint, kubeconfig, status, version, description, created_by, create_time, update_time FROM devops_cluster %s ORDER BY id DESC LIMIT %d, %d", where, offset, pageSize)
	if err := l.svcCtx.SqlConn.QueryRowsCtx(l.ctx, &clusters, listQuery, args...); err != nil {
		return nil, err
	}

	var list []*devops.ClusterItem
	for _, c := range clusters {
		list = append(list, &devops.ClusterItem{
			Id:          c.Id,
			Name:        c.Name,
			Env:         c.Env,
	...
	return &devops.ListClustersResp{
		Total: total,
		List:  list,
	}, nil
}
