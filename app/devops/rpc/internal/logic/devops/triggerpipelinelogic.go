...
func (l *TriggerPipelineLogic) TriggerPipeline(in *devops.TriggerPipelineReq) (*devops.TriggerPipelineResp, error) {
	...
	res, err := l.svcCtx.PipelineExecModel.Insert(l.ctx, &model.DevopsPipelineExec{
		PipelineId:    p.Id,
	...
	for _, stage := range stages {
		for _, step := range stage.Steps {
			_, _ = l.svcCtx.PipelineStepExecModel.Insert(l.ctx, &model.DevopsPipelineStepExec{
				ExecId:    execId,
				StageId:   stage.ID,
				StepId:    step.ID,
				StepName:  step.Name,
	...
	return &devops.TriggerPipelineResp{
		ExecId: execId,
		ExecNo: execNo,
	}, nil
}
