...
func (l *GetExecutionDetailLogic) GetExecutionDetail(in *devops.GetExecutionDetailReq) (*devops.ExecutionDetailResp, error) {
	...
	var stepExecs []*model.DevopsPipelineStepExec
	query := fmt.Sprintf("SELECT id, exec_id, stage_id, step_id, step_name, step_type, status, log_path, error_msg, start_time, end_time, duration_ms, create_time FROM devops_pipeline_step_exec WHERE exec_id = ? ORDER BY id ASC")
	_ = l.svcCtx.SqlConn.QueryRowsCtx(l.ctx, &stepExecs, query, in.ExecId)

	var stepList []*devops.StepExecItem
	for _, s := range stepExecs {
		...
		stepList = append(stepList, &devops.StepExecItem{
			Id:         s.Id,
			ExecId:     s.ExecId,
			StageId:    s.StageId,
			StepId:     s.StepId,
	...
	return &devops.ExecutionDetailResp{
		Execution: execItem,
		Steps:     stepList,
	}, nil
}
