  ...
  const serviceDetails = [
    {
      ...
      desc: "作为大仓唯一对外暴露的 HTTP 流量入口（端口 8888），承载统一路由、JWT Auth 身份解析、参数校验、跨微服务数据聚合与统一响应封装。",
      tags: ["HTTP 8888", "RESTful", "JWT Auth", "mr.Finish 聚合"],
    },
    {
      title: "用户中心微服务 (User RPC)",
      icon: <ClusterOutlined style={{ fontSize: 24, color: "#1677ff" }} />,
      desc: "纯 gRPC 业务微服务（端口 8080），承载企业员工账号、角色管理、组织架构部门树、动态菜单树与按钮权限下发，以及角色权限一石二鸟事务闭环。",
    ...
    },
    {
      ...
      desc: "由 goctl model 从 MySQL DDL 逆向生成的带 Cache-Aside 机制 Go Model。单条记录走 Redis 缓存，更新与删除精准淘汰，多表变更严格由 TransactCtx 事务保证原子性。",
      tags: ["MySQL 8.0", "Redis 缓存", "Cache-Aside", "TransactCtx"],
    },
    {
      title: "服务注册与发现 (Nacos / Etcd)",
      icon: <SafetyCertificateOutlined style={{ fontSize: 24, color: "#eb2f96" }} />,
      desc: "原生支持 Nacos（默认）、Etcd 以及本地直连（Endpoints）三种寻址与治理模式，零代码改动，通过 YAML 配置文件一键无缝切换。",
      tags: ["Nacos 2.x", "Etcd 3.5", "平滑寻址", "零代码切换"],
    },
    {
      title: "现代化工具链与规范 (Toolchain)",
      icon: <ToolOutlined style={{ fontSize: 24, color: "#13c2c2" }} />,
      desc: "由 mise 统一锁定 Go、Node、pnpm、goctl、protoc、just、antd-cli 版本；搭配 antd lint 静态诊断与代码知识图谱引擎，保障工程持续高可靠。",
