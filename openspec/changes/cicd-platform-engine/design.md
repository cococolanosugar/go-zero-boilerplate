...
**Goals:**
- **架构解耦与职责收敛**：新增 `app/devops/rpc`（Titan 微服务核心，gRPC :8086）独立微服务，承载 DevOps 领域规则与数据持久化，网关仅做路由聚合；
...
### 1. 微服务职责划分与网关契约
- **决策**：新建 `app/devops/rpc` 纯 gRPC 服务（端口 8086），网关 `app/gateway/desc/devops.api` 暴露标准 RESTful API；前端通过 `just gen-ts` 生成 `@zero/api` 强类型调用函数。
- **替代方案考虑**：
...
- **架构原理**：
  - 将一次 Pipeline 运行映射为一个 Temporal Workflow (`PipelineExecutionWorkflow`)；
...
### 5. 数据模型设计 (5 张核心数据表)
1. `devops_integration`：集成配置表（Jenkins、Harbor/ACR、Git 凭据，敏感凭据 AES-GCM 加密存储）；
2. `devops_cluster`：Kubernetes 集群表（集群名称、环境标识、API Endpoint、KubeConfig 密文、状态）；
3. `devops_pipeline`：流水线定义表（项目 ID、流水线名称、触发方式、阶段与步骤编排 DSL JSON）；
4. `devops_pipeline_exec`：流水线运行记录表（执行编号、触发人、Git 分支/Commit、全局参数、运行状态、耗时、产物列表）；
5. `devops_pipeline_step_exec`：步骤执行记录表（对应 Activity、步骤类型、状态、开始/结束时间、日志存储路径/Offset）。
...
- **[Kubernetes 集群发布权限越权]** → **缓解方案**：纳管 K8s 集群凭证时支持只赋予指定命名空间（Namespace）的 RBAC 角色，发布时严格校验目标 Namespace 与集群是否在授权清单内。

## Migration Plan

1. 执行数据库迁移脚本（通过 `just migrate-new` 生成 `create_devops_tables` 并执行 `just migrate-up`）；
2. 启动 `devops-rpc` 微服务并在 Nacos/直连配置中挂载至网关 `etc/gateway.yaml`；
3. 执行 `just gen-gateway` 与 `just gen-ts` 保持全栈契约同步；
4. 部署 `admin` 前台 `/devops` 路由组件，并验证 Jenkins 与 K8s 集群连通性。
